...
<head>
    ...
    <link rel="stylesheet" href="/css/contents.css"/>
      <link rel="stylesheet" href="//code.jquery.com/ui/1.13.2/themes/base/jquery-ui.css"><!-- 제이쿼리 css -->
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.6.0/jquery.min.js"></script>
      <script src="https://code.jquery.com/ui/1.13.2/jquery-ui.js"></script><!-- 제이쿼리 ui -->
    ...
    function goSave(){
    	if($("#email").val().trim()==''){
    		alert('이메일 입력해주세요');
    		$("#email").focus();
    		return;//return;을 넣어줘야 전송이 안됨, return의 중지 기능
    	}
   	 ...
   	  var reg_email = /^([0-9a-zA-Z_\.-]+)@([0-9a-zA-Z_-]+)(\.[0-9a-zA-Z_-]+){1,2}$/;
   	  if(!reg_email.test($("#email").val())) {                            
   		 ...
   		 //중복체크 했는지 확인 필요, 체크는 했지만 아직 가입하기는 유무없이 넘어감
   		 var isCon = true;
   		 	$.ajax({
    			url:'emailDupCheck.do',
    			data:{email:$("#email").val() },
    			async:false,//ajaz 비동기 통신을 > 동기 통신으로 설정
    			success: function(res){
    				if(res=='true'){
    					...
    					$("#email").focus();
    					isCon=false;
    					//return;//이 return;은 ajax안에서 중지됨
    				}
    			}
    		}) ;
   		 //****중요 직접처리가 안되서, 간접적으로 바꿈
   		 if(!isCon)return;
