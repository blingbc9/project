...
<p><span><strong>총 ${comment.totalCount }개</strong>  |  ${commentVO.page} / ${comment.totalPage}</span></p>
	                    <table class="list">
	                        <colgroup>
	                            <col width="80px" />
	                            <col width="*" />
		                            ...
		                            <c:forEach var="vo" items="${ comment.list}"> 
			                            <tr>
			                                ...
			                                <td class="txt_l" style="text-align:left;" >
			                                    ${vo.content }<c:if test="${ loginInfo.no ==vo.memberno}"><a href="javascript:commentDel(${vo.no });">[삭제]</a></c:if>
			                                </td>
			                                <td class="writer">
			                                    ${vo.member_name }
			                                </td>
			               ...
			               <ul class='paging'>
				               <c:if test="${comment.prev ==true }">
				               	...
				               	<c:forEach var="p" begin="${comment.startPage}" end="${comment.endPage}">
				               		<li><a href ='javascript:getComment(${p });' <c:if test="${commentVO.page == p}">class='current'</c:if>>${p}</a></li></a>
				               	</c:forEach>
